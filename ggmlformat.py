...
def ggml_conversion(repo_name):
    print(repo_name)
    if os.path.isdir('whisper')==False:
        git.Git().clone('https://github.com/openai/whisper')
        
    if os.path.isdir('whisper.cpp')==False:
        git.Git().clone('https://github.com/ggerganov/whisper.cpp')
        
    #.Git().clone('https://huggingface.co/Shubham09/LISA_Whisper_small_latest')
    #os.system('python3 ./whisper.cpp/models/convert-h5-to-ggml.py' + './' +repo_name+'/' './whisper .')
    ...
    dir_model   =repo_name   #sys.argv[1] 
    dir_whisper = "whisper"
    dir_out     = repo_name
    print(dir_model)
    ...
    fname_out = dir_out + "_ggml-model.bin"
    ...
    blob_client = blob_service_client.get_blob_client(container='offlinespeech2text', blob='compressed model/ggml/' + fname_out)
    with open(fname_out, "rb") as data:
        blob_client.upload_blob(data, overwrite=True)
        print("uploading file ---->", fname_out)
    if os.path.isdir('whisper')==True:
        shutil.move('whisper', tmpdirname)    
    if os.path.isdir('whisper.cpp')==True:
        shutil.move('whisper.cpp', tmpdirname)

    if fname_out:
        return ("converted to ggml format and uploaded to Blob successfully")
